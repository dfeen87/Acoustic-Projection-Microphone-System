...
float BeamformingEngine::lagrange_interpolate(std::span<const float> signal, float idx) const {
    const int order = 3;
...
AudioFrame BeamformingEngine::delay_and_sum(
    ...
    for (int m = 0; m < array_size_; ++m) {
        ...
        delays[m] = (pos * std::sin(azimuth_rad) * std::cos(elevation_rad)) / speed_of_sound_;
        delays[m] *= sr; // Convert to samples
    }
    
    // Apply delays and sum
    for (size_t i = 0; i < frame_size; ++i) {
        float sum = 0.0f;
        for (int m = 0; m < std::min(array_size_, static_cast<int>(mic_array.size())); ++m) {
            float delayed_idx = static_cast<float>(i) - delays[m];
            if (delayed_idx >= 0 && delayed_idx < mic_array[m].frame_count()) {
...
std::vector<float> NoiseSuppressionEngine::compute_features(const std::vector<float>& frame) {
...
std::vector<float> NoiseSuppressionEngine::lstm_forward(const std::vector<float>& input) {
...
AudioFrame EchoCancellationEngine::cancel_echo(
    ...
    size_t min_size = std::min(mic_samples.size(), ref_samples.size());
...
float VoiceActivityDetector::compute_energy_db(const AudioFrame& frame) const {
    float energy = 0.0f;
...
int VoiceActivityDetector::compute_zero_crossing_rate(const AudioFrame& frame) const {
    int crossings = 0;
...
VoiceActivityDetector::VadResult VoiceActivityDetector::detect(const AudioFrame& frame) {
...
void VoiceActivityDetector::adapt_threshold(float ambient_noise_db) {
...
std::future<TranslationEngine::TranslationResult> MockTranslationEngine::translate_async(
...
DirectionalProjector::DirectionalProjector(int speakers, float spacing)
    : speaker_array_size_(speakers), spacing_m_(spacing) {}

std::vector<AudioFrame> DirectionalProjector::create_projection_signals(
...
std::future<std::vector<AudioFrame>> APMSystem::process_async(
